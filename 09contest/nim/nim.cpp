...
int main(int argc, char *argv[]) {
  ...
  setvbuf(stdin, inputbuffer, _IOFBF, BUFFER_SIZE);
	...
	rep(game, games_count) {
		...
		bool is_l[letters_count];

		rep(letter, letters_count) {
			char c = getchar_unlocked();
			is_l[letter] = (c == 'l');
		}
		getchar_unlocked();

		if (!(is_l[0] == true && is_l[letters_count - 1] == false)) {
			write(0);
			continue;
		}
		...
		for(; is_l[left_l]; left_l++);
		for(int i = letters_count - 1; !is_l[i]; i--) {
			right_r++;
		}

		write(min(left_l, right_r));
	}
